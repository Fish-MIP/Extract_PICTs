...
We will need to create a raster mask for each unique grid. In this case, IPSL and GFDL share the same 60 arc minute ($1^\circ$) grid, but IPSL inputs are also available at a 120 arc minute ($2^{\circ}$) resolution. This means that we will need to create two different masks.   
