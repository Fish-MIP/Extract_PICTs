...
server <- function(input, output) {
  ...
  filtered_data <- reactive({
    ...
    #file_to_get<-paste("../Data/apecosm_ipsl-cm6a-lr_nobasd_historical_nat_default_tcb_mean-yearly_1850_2014.nc")
    
    # if (input$variable=="tcblog10" & input$model %in% c("DBPM","APECOSM","BOATS","ZooMSS","MACROECOLOGICAL","EcoTroph")) {gridded_ts <- sum(brick(file.path(file_to_get),level=2),brick(file.path(file_to_get),level=2),brick(file.path(file_to_get),level=3),brick(file.path(file_to_get),level=4),brick(file.path(file_to_get),level=5)) } 
    # 
    # if (input$variable!="tcblog10") gridded_ts <- brick(file.path(file_to_get))
    # 
    ...
    gridded_ts <- mask(temp, mask_to_use)
     ...
     # aggregate the stack to annual means
    # yearly_s = terra::tapp(gridded_ts, index = 1850:2014, fun = mean)
    # yearly_s = brick(yearly_s)
    # return(yearly_s)
